...
router.post('/', (req, res) => {
  ...
  pool.query('SELECT * FROM xfn_category WHERE cname= ?', data.cname, (err, result) => {
    if (err) throw err;
    if (result.length > 0) {
      res.send({ code: 401, msg: 'this category existed' });
    } else {
      pool.query('INSERT INTO xfn_category SET ?', data, (err, result) => {//此处sql语句简写
        if (err) throw err;
        if (result.affectedRows > 0) {
...
*/
router.put('/',(req,res)=>{
  var data=req.body;//数据{cid:xx,cname:'xx'}
  //此处可以对数据进行验证
  pool.query('UPDATE xfn_category SET ? WHERE cid=?',[data,data.cid],(err,result)=>{
    if(err)throw err;
    if(result.changedRows>0){  //修改了一行
      res.send({code:200,msg:'1 category modified'});
    }else if(result.affectedRows==0){//影响了0行，表示没有改类别
      res.send({code:400,msg:'category not exist'});
    }else if(result.affectedRows==1 && result.changedRows==0){//影响到1行，但修改了0行，新值与旧值一样
